...
export const checkValidCategory = async (data, categoryType) => new Promise(async (resolve, reject) => {
    try {
        const category = await db.category.findOne({ purrPetCode : data.categoryCode });
        if (category === null || category.categoryName != data.categoryName || category.categoryType != categoryType) {
            return resolve({
                err: -1,
